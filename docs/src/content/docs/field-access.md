...
const sireflect_field_info_t *field =
    sireflect_field_info(position_type, "x");
...
sireflect_handle_t field_type =
    sireflect_field_type(position_type, "x");

size_t field_size =
    sireflect_field_size(position_type, "x");
```
...
```c
const f32 *x = sireflect_field_ptr(position_type, &position, "x");
```
...
```c
f32 *x = sireflect_field_mut_ptr(position_type, &position, "x");
*x = 12.0f;
```
...
if (sireflect_field_copy(position_type, &position, "x", &value) != 0) {
    /* Missing field. */
}
```
...
```c
static void print_type_fields(sireflect_handle_t type) {
    ...
    for (size_t i = 0; i < fields->field_count; i++) {
        ...
        const char *type_name = sireflect_type_name(field->type);
